...
static bool firstImageRetrieved;
...
-(void) setup {
    ...
    NSLog(@"%s pagenum = %d",__PRETTY_FUNCTION__,(int)pn);
    self.pagenum = (int)pn;
...
-(void) prevPages:(id)sender {
    ...
    _pagenum = ptmp;
    _pagenum--;
    pageNumTextField.text = [NSString stringWithFormat:@"%ld",_pagenum];
    [self loadPage];
}
-(void) nextPages:(id)sender {
    ...
    _pagenum = ptmp;
    _pagenum++;
    pageNumTextField.text = [NSString stringWithFormat:@"%ld",(long)_pagenum];
    [self loadPage];
}
...
-(void) addPager {
    ...
    nextbutton.tag = 0;
    nextbutton.frame = CGRectMake(self.bounds.size.width-buttonwidth,6,buttonwidth,buttonheight);
    ...
    CGFloat spacer = 2.0;
    pageNumTextField = [[UITextField alloc]initWithFrame:CGRectMake(nextbutton.frame.origin.x-tfwidth-spacer,6,tfwidth,buttonheight)];
    ...
    prevbutton.frame = CGRectMake(pageNumTextField.frame.origin.x - spacer-buttonwidth,6,buttonwidth,buttonheight);
...
- (void) retrievePage {
    ...
    NSURL *xmlURL = [NSURL URLWithString:[NSString stringWithFormat:@"http://glslsandbox.com/?page=%d",self.pagenum]];
...
-(void) loadPage {
    ...
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"http://glslsandbox.com/?page=%d",self.pagenum]];
    ...
    [NSURLConnection sendAsynchronousRequest:request
                           ...
                           completionHandler:^(NSURLResponse *_response, NSData *_responseData, NSError *_error) {
                               ...
                               @catch (NSException *exception) {
                                   NSLog(@"Exception: %s %@",__PRETTY_FUNCTION__,exception.debugDescription);
                               }
                               @finally {
                               }
...
-(void) createImage:(NSString*) src {
    ...
    NSURL *imageUrl = [NSURL URLWithString:[NSString stringWithFormat:@"http://glslsandbox.com/%@",src]];
                                               ...
                                               NSError *_reqerror)
     {
         ...
         if(_reqerror == nil)
         {
             ...
             dispatch_async(dispatch_get_main_queue(), ^{
                 NSURL *url = [_urlresponse URL];
                 NSString *lpc = [url lastPathComponent];
                 NSString *imagenum = [lpc stringByReplacingOccurrencesOfString:@".png" withString:@""];
                 [self createImageButton:img tag:atof([imagenum UTF8String]) ];
             });
         }
         else
         {
             NSLog(@"erorr on image download");
         }
     }];
...
-(void) createImageButton:(UIImage*) img tag:(int) tag {
    ...
    if( ! firstImageRetrieved ) {
        [self.delegate setSliderRange:lastId];
        firstImageRetrieved = YES;
    }
    
    defaultbutton.tag = tag;  // appears right but async could fuck it up
